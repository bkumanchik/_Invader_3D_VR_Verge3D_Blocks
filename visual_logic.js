/**
 * Generated by Verge3D Puzzles v.4.0.0 pre3
 * Thu Jun 23 2022 08:07:02 GMT-0700 (Pacific Daylight Time)
...
var game_over, turret_fired, invader_blast_pause, invader_been_hit, score, turret_exploding, turret_been_hit, lives, turret_check_x, invader_fired, inv_shot_z, controller_l, inv_dir, fire_pressed, inv_shot_x, increasing__score, controller_r, shot_z, shot_x, shot_y, explosion_sound, boom_sound, pew_sound, inv_x, inv_y, inv_z;
...
function initialize() {
  ...
  inv_x = getObjTransform('invader_root', false, 'position', 'x');
  inv_y = getObjTransform('invader_root', false, 'position', 'y');
  inv_z = getObjTransform('invader_root', false, 'position', 'z');
  ...
  setObjTransform('invader_root', false, 'position', [0, 10.6, 2.62], false);
  setObjTransform('turret_root', false, 'position', [0, 10.6, -3.6], false);
  controller_l = 0;
  controller_r = 1;
}
  ...
  if (!invader_blast_pause && !game_over) {
    registerSetTimeout(0.5, function() {
      ...
      registerSetTimeout(0.5, function() {
        ...
        if (getObjTransform('invader_root', false, 'position', 'x') > 3.8) {
          if (inv_dir == 0.1) {
            inv_dir = -0.1;
  ...
  if (!game_over) {
    registerSetTimeout(3, function() {
      if (!invader_been_hit) {
        if (!invader_fired) {
          invader_fired = true;
          inv_shot_x = getObjTransform('invader_root', false, 'position', 'x');
          inv_shot_z = getObjTransform('invader_root', false, 'position', 'z');
          setObjTransform('inv_shot', false, 'position', [inv_shot_x, '', inv_shot_z], false);
...
'Invader VR  by Brian Kumanchik started 7/9/2021';

registerOnClick('enter_vr_button', false, false, [0,1,2], function() {
  enterVRMode('ORIGIN', function() {
    changeVis('enter_vr_button', false);
    initialize();
  }, function() {}, function() {});
}, function() {});
...
registerEveryFrame(function() {
  if (!turret_exploding && !game_over) {
    if (getGamepadProp(controller_l, 'AXIS', '2') < -0.3) {
      turret_check_x = getObjTransform('turret_root', false, 'position', 'x');
...
registerEveryFrame(function() {
  if (!turret_exploding && !game_over) {
    if (getGamepadProp(controller_r, 'BUTTON', '4') && !fire_pressed) {
      fire_pressed = true;
      if (!turret_fired) {
        turret_fired = true;
        shot_x = getObjTransform('turret_root', false, 'position', 'x');
        shot_y = getObjTransform('turret_root', false, 'position', 'y');
        shot_z = getObjTransform('turret_root', false, 'position', 'z');
        setObjTransform('shot', false, 'position', [shot_x, shot_y, shot_z], false);
...
registerEveryFrame(function() {
  if (turret_fired) {
    ...
    shot_z = getObjTransform('shot', false, 'position', 'z');
    if (shot_z > 3) {
      changeVis('shot', false);
      shot_x = getObjTransform('turret_root', false, 'position', 'x');
      shot_y = getObjTransform('turret_root', false, 'position', 'y');
...
registerEveryFrame(function() {
  if (!getGamepadProp(controller_r, 'BUTTON', '4')) {
    fire_pressed = false;
  }
});
...
registerEveryFrame(function() {
  if (invader_fired && !game_over) {
    ...
    inv_shot_z = getObjTransform('inv_shot', false, 'position', 'z');
    if (inv_shot_z < -3.3) {
      changeVis('inv_shot', false);
      inv_shot_x = getObjTransform('invader_root', false, 'position', 'x');
      inv_shot_z = getObjTransform('invader_root', false, 'position', 'z');
...
detectCollision('shot_proxy', 'invader_proxy', function() {
  if (!invader_blast_pause && !game_over) {
    ...
    setObjTransform('shot', false, 'position', [0, -5, ''], false);
...
detectCollision('inv_shot_proxy', 'turret_proxy', function() {
  if (!turret_exploding && !game_over) {
    ...
    changeVis('turret', false);
    changeVis('tur_exp_a', true);
    setObjTransform('inv_shot', false, 'position', [1, '', 5], false);
...
registerEveryFrame(function() {
  if (game_over) {
    if (getGamepadProp(controller_l, 'BUTTON', '4')) {
      game_over = false;
      initialize();
    }
  }
