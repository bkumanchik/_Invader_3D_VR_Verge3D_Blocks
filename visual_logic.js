 ...
 * Thu Jul 22 2021 10:22:47 GMT-0700 (Pacific Daylight Time)
...
function initialize() {
  ...
  inv_z = getObjTransform('invader_root', 'position', 'z');
  ...
  setObjTransform('invader_root', 'position', 0, 10.6, 2.62, false);
  setObjTransform('turret_root', 'position', 0, 10.6, -3.6, false);
  controller_l = 0;
  controller_r = 1;
}
  ...
  if (!game_over) {
    registerSetTimeout(3, function() {
      if (!invader_been_hit) {
        if (!invader_fired) {
          invader_fired = true;
          inv_shot_x = getObjTransform('invader_root', 'position', 'x');
          inv_shot_z = getObjTransform('invader_root', 'position', 'z');
          setObjTransform('inv_shot', 'position', inv_shot_x, '', inv_shot_z, false);
  ...
  if (!turret_exploding && !game_over) {
    if (getGamepadProp(controller_r, 'BUTTON', '4') && !fire_pressed) {
      ...
      if (!turret_fired) {
        ...
        shot_z = getObjTransform('turret_root', 'position', 'z');
        setObjTransform('shot', 'position', shot_x, shot_y, shot_z, false);
...
registerEveryFrame(function() {
  if (turret_fired) {
    ...
    shot_z = getObjTransform('shot', 'position', 'z');
    if (shot_z > 3) {
...
registerEveryFrame(function() {
  if (invader_fired && !game_over) {
    ...
    inv_shot_z = getObjTransform('inv_shot', 'position', 'z');
    if (inv_shot_z < -3.3) {
